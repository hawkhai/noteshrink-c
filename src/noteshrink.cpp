...
static float KMeans(std::vector<NSHRgb> const& data, int k, int maxItr, //
    std::vector<NSHRgb>& means, std::vector<int>& counts)
{
    means.clear();
    means.reserve(k);
    counts.clear();
...
static void CreatePalette(std::vector<NSHRgb> const& samples, NSHOption option, std::vector<NSHRgb>& outPalette, NSHRgb& bgColor)
{
    ...
    float cluse = KMeans(samples, outPalette.size() - 1, option.KmeansMaxIter, means, counts);
